...
// Validação
async function validar_frm_militares_auxilios_fardamentos() {
    ...
    // Campo: auxilio_fardamento_tipo_id (requerido)
    if (validacao({ op: 1, value: document.getElementById("auxilio_fardamento_tipo_id").value }) === false) {
        validacao_ok = false;
        mensagem += "Auxílio Fardamento Tipo requerido." + "<br>";
    }
...
async function settingsSubmoduloCrudCreate() {
    ...
    // Pesquisar Militar
    const militarAuto = crudAutocompleteMilitar('militares_auxilios_fardamentos', 'create');
    militarAuto.init();
...
async function settingsSubmoduloCrudView(data) {
    ...
    await configurarBotoesPermissaoSituacao({ modulo: 'militares_auxilios_fardamentos', situacaoId: data.militarSituacaoId });
