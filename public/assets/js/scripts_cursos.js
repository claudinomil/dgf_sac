...
// Validação
async function validar_frm_cursos() {
    ...
    // Campo: abreviacao (requerido)
    if (validacao({ op: 1, value: document.getElementById("abreviacao").value }) === false) {
        validacao_ok = false;
        mensagem += "Abreviação requerido." + "<br>";
    }

    // Campo: oficial_praca (requerido)
    if (validacao({ op: 1, value: document.getElementById("oficial_praca").value }) === false) {
        validacao_ok = false;
        mensagem += "Oficial/Praça requerido." + "<br>";
    }

    // Campo: percentual (requerido)
    if (validacao({ op: 1, value: document.getElementById("percentual").value }) === false) {
        validacao_ok = false;
        mensagem += "Percentual requerido." + "<br>";
    }
