...
// Validação
async function validar_frm_militares_cursos() {
    ...
    // Campo: curso_id (requerido)
    if (validacao({ op: 1, value: document.getElementById("curso_id").value }) === false) {
        validacao_ok = false;
        mensagem += "Curso requerido." + "<br>";
    }

    // Campo: data_inicio (requerido)
    if (validacao({ op: 1, value: document.getElementById("data_inicio").value }) === true) {
    ...
    if (validacao({ op: 1, value: document.getElementById("data_termino").value }) === false) {
        validacao_ok = false;
        mensagem += "Data Término requerido." + "<br>";
    } else {
        if (validacao({ op: 8, value: document.getElementById('data_termino').value }) === false) {
            validacao_ok = false;
            mensagem += 'Data Término inválida.' + '<br>';
        }
    }
    ...
    // Campo: conceito (requerido)
    if (validacao({ op: 1, value: document.getElementById("conceito").value }) === false) {
        validacao_ok = false;
        mensagem += "Conceito requerido." + "<br>";
    }

    // Campo: classificacao (requerido)
    if (validacao({ op: 1, value: document.getElementById("classificacao").value }) === false) {
        validacao_ok = false;
        mensagem += "Classificação requerido." + "<br>";
    }
...
async function settingsSubmoduloCrudCreate() {
    ...
    // Pesquisar Militar
    const militarAuto = crudAutocompleteMilitar('militares_cursos', 'create');
    militarAuto.init();
...
async function settingsSubmoduloCrudView(data) {
    ...
    await configurarBotoesPermissaoSituacao({ modulo: 'militares_cursos', situacaoId: data.militarSituacaoId });
